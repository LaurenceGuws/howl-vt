...
pub const graphics_payload_max_bytes: u32 = 65 * 1024 * 1024;
pub const upload_max_bytes: u32 = graphics_payload_max_bytes;
pub const decoded_payload_max_bytes: u32 = graphics_payload_max_bytes;
...
pub const State = struct {
    ...
    pub fn progressRuntime(self: *State, allocator: std.mem.Allocator, now_ns: u64) host_state.ApplyError!bool {
        ...
        return self.progressImageRuntime(@intCast(idx), now_ns);
    }
    ...
    fn progressImageRuntime(self: *State, image_idx: Index, now_ns: u64) bool {
        ...
        if (image.current_frame_shown_at_ns == 0) {
            ...
            return self.advanceRuntimeFrame(image_idx, now_ns);
        }

        if (gap > 0) {
            const gap_ns = gapToNs(gap) orelse return self.advanceRuntimeFrame(image_idx, now_ns);
    ...
    fn advanceRuntimeFrame(self: *State, image_idx: Index, now_ns: u64) bool {
        const image = &self.images.items[@intCast(image_idx)];
    ...
    pub fn imageAt(self: *const State, idx: Index) ?Image {
        ...
        assertImagePayloadsCoherent(image);
        return image;
    }
    ...
    fn controlAnimation(self: *State, allocator: std.mem.Allocator, output: *std.ArrayList(u8), encode_buf: []u8, cmd: KittyGraphicsCommand) host_state.ApplyError!HandleResult {
        ...
        if (has_current_frame) {
            changed = self.selectCurrentFrame(@intCast(image_idx), cmd.current_frame_number) or changed;
        }
    ...
    fn composeFrame(self: *State, allocator: std.mem.Allocator, output: *std.ArrayList(u8), encode_buf: []u8, cmd: KittyGraphicsCommand) host_state.ApplyError!HandleResult {
        ...
        try self.ensureDecodedPayloadStore(
            allocator,
            count32(dest),
            if (dest_frame_number == 1) decodedPayloadBytesFreedByPublishedRoot(self, image_id) else decodedPayloadBytesFreedByFrame(self, image_id, dest_frame_number),
            image_id,
        );
        ...
        assertImagePayloadsCoherent(self.images.items[@intCast(self.findImage(image_id) orelse return .{ .changed = true, .move = null })]);
        if (dest_frame_number != 1) {
            const frame_idx = self.findFrameIndex(image_id, dest_frame_number) orelse unreachable;
            assertFramePayloadsCoherent(self.frames.items[@intCast(frame_idx)]);
        }
    ...
    fn appendUploadChunk(self: *State, allocator: std.mem.Allocator, screen: *const screen_mod.Screen, render_view: RenderCursorView, cell_pixel_size: ?CellPixelSize, output: *std.ArrayList(u8), encode_buf: []u8, cmd: KittyGraphicsCommand, more: bool) host_state.ApplyError!?CursorMove {
        ...
        if (self.upload) |*upload| {
            ...
            try self.ensureDecodedPayloadStore(allocator, 0, 0, null);
            ...
            self.upload = null;
            const owned = normalizeDirectTransportOwned(allocator, compression, format, width, height, decoded_transport) catch |err| {
                switch (err) {
                    error.InvalidGraphicsCompression => {
    ...
    fn storeImageOwned(self: *State, allocator: std.mem.Allocator, image_id: u32, image_number: u32, format: u16, width: u32, height: u32, owned: []u8) host_state.ApplyError!void {
        ...
        try self.ensureDecodedPayloadStore(
            allocator,
            std.math.cast(u32, decoded_len) orelse return error.ConsequenceLimit,
            decodedPayloadBytesFreedByImage(self, image_id),
            if (image_id == 0) null else image_id,
        );
        ...
        std.debug.assert(decodedPayloadBytes(self) <= decoded_payload_max_bytes);
    }
    ...
    fn storeFrameOwned(self: *State, allocator: std.mem.Allocator, image_id: u32, frame_number: u32, format: u16, width: u32, height: u32, x: u32, y: u32, base_frame_number: u32, compose_mode: u32, background_rgba: u32, gap: i32, owned: []u8) host_state.ApplyError!void {
        ...
        if (!framePublicationFormatSupported(image.format)) return;
        if (!framePublicationFormatSupported(format)) return;
        if (target_frame_number == 1) {
            try self.editRootFrame(
                allocator,
                @intCast(image_idx),
                format,
                frame_size.width,
        ...
        try self.ensureDecodedPayloadStore(
            allocator,
            std.math.cast(u32, decoded_len) orelse return error.ConsequenceLimit,
            decodedPayloadBytesFreedByFrame(self, image_id, target_frame_number),
            image_id,
        );
        ...
        if (self.findFrameIndex(image_id, target_frame_number)) |existing_idx| {
            ...
            assertFramePayloadsCoherent(frame.*);
    ...
    fn editRootFrame(
        ...
        const over_decoded = try decodedPublicationOwned(allocator, format, width, height, payload);
        defer allocator.free(over_decoded.payload);
        const over = if (target_format == 32)
            try decodedRgbaOwned(allocator, format, width, height, over_decoded.format, over_decoded.width, over_decoded.height, over_decoded.payload)
        else
            try decodedRawOwned(allocator, format, width, height, over_decoded.format, over_decoded.width, over_decoded.height, over_decoded.payload);
        ...
        try self.ensureDecodedPayloadStore(allocator, count32(root), decodedPayloadBytesFreedByPublishedRoot(self, image_id), image_id);
    ...
    fn selectCurrentFrame(self: *State, image_idx: Index, frame_number: u32) bool {
        const image = &self.images.items[@intCast(image_idx)];
    ...
    fn coalesceFrameNumberOwned(self: *const State, allocator: std.mem.Allocator, image: Image, frame_number: u32, target_format: u16) host_state.ApplyError![]u8 {
        ...
        if (frame_number == 1) {
            return if (target_format == 32)
                try decodedRgbaOwned(allocator, image.format, image.width, image.height, image.decoded_format, image.decoded_width, image.decoded_height, image.decoded_payload)
            else
                try decodedRawOwned(allocator, image.format, image.width, image.height, image.decoded_format, image.decoded_width, image.decoded_height, image.decoded_payload);
        }
        const frame = self.frameByNumber(image.image_id, frame_number) orelse unreachable;
        return if (target_format == 32)
    ...
    fn coalesceFrameRawOwned(self: *const State, allocator: std.mem.Allocator, image: Image, frame: Frame) host_state.ApplyError![]u8 {
        assertImagePayloadsCoherent(image);
        assertFramePayloadsCoherent(frame);
        const root = try decodedRawOwned(allocator, image.format, image.width, image.height, image.decoded_format, image.decoded_width, image.decoded_height, image.decoded_payload);
    ...
    fn coalesceFrameRgbaOwned(self: *const State, allocator: std.mem.Allocator, image: Image, frame: Frame) host_state.ApplyError![]u8 {
        assertImagePayloadsCoherent(image);
        assertFramePayloadsCoherent(frame);
        const root = try decodedRgbaOwned(allocator, image.format, image.width, image.height, image.decoded_format, image.decoded_width, image.decoded_height, image.decoded_payload);
        if (frame.uses_root_base) {
    ...
    fn composeStandaloneFrameOwned(self: *const State, allocator: std.mem.Allocator, image: Image, frame: Frame, root: []u8) host_state.ApplyError![]u8 {
        ...
        if (frame.x == 0 and frame.y == 0 and frame.width == image.width and frame.height == image.height and frame.background_rgba == 0) {
            allocator.free(root);
            return try decodedRawOwned(allocator, frame.format, frame.width, frame.height, frame.decoded_format, frame.decoded_width, frame.decoded_height, frame.decoded_payload);
        }
        const base = try allocateBackgroundRaw(allocator, frame.format, image.width, image.height, frame.background_rgba);
        errdefer allocator.free(base);
        const over = try decodedRawOwned(allocator, frame.format, frame.width, frame.height, frame.decoded_format, frame.decoded_width, frame.decoded_height, frame.decoded_payload);
        defer allocator.free(over);
    ...
    fn composeStandaloneFrameRgbaOwned(self: *const State, allocator: std.mem.Allocator, image: Image, frame: Frame, root: []u8) host_state.ApplyError![]u8 {
        ...
        if (frame.x == 0 and frame.y == 0 and frame.width == image.width and frame.height == image.height and frame.background_rgba == 0) {
            allocator.free(root);
            return try decodedRgbaOwned(allocator, frame.format, frame.width, frame.height, frame.decoded_format, frame.decoded_width, frame.decoded_height, frame.decoded_payload);
        }
        const base = try allocateBackgroundRaw(allocator, 32, image.width, image.height, frame.background_rgba);
        errdefer allocator.free(base);
        const over = try decodedRgbaOwned(allocator, frame.format, frame.width, frame.height, frame.decoded_format, frame.decoded_width, frame.decoded_height, frame.decoded_payload);
        defer allocator.free(over);
    ...
    fn ensureDecodedPayloadStore(self: *State, allocator: std.mem.Allocator, next_len: u32, freed_len: u32, protected_image_id: ?u32) host_state.ApplyError!void {
        if (next_len > decoded_payload_max_bytes) return error.ConsequenceLimit;
        if (!self.decodedPayloadFits(next_len, freed_len)) {
            self.evictUnplacedImagesForDecodedPayload(allocator, next_len, freed_len, protected_image_id);
        }
        if (!self.decodedPayloadFits(next_len, freed_len)) return error.ConsequenceLimit;
        std.debug.assert(self.decodedPayloadFits(next_len, freed_len));
    }
...
fn assertImagePayloadsCoherent(image: Image) void {
    assertDecodedPayloadCoherent(
        image.format,
        image.width,
        image.height,
        image.decoded_format,
        image.decoded_width,
        image.decoded_height,
        image.decoded_payload,
    );
}

fn assertFramePayloadsCoherent(frame: Frame) void {
    assertDecodedPayloadCoherent(
        frame.format,
        frame.width,
        frame.height,
...
fn assertDecodedPayloadCoherent(format: u16, width: u32, height: u32, decoded_format: u16, decoded_width: u32, decoded_height: u32, payload: []const u8) void {
    std.debug.assert(payload.len <= decoded_payload_max_bytes);
    std.debug.assert(width > 0);
    std.debug.assert(height > 0);
    std.debug.assert(decoded_width == width);
    std.debug.assert(decoded_height == height);
    if (format == 100) {
        std.debug.assert(decoded_format == 32);
    } else {
        std.debug.assert(format == 24 or format == 32);
        std.debug.assert(decoded_format == format);
    }
    const decoded_len = expectedRawPayloadLen(decoded_format, decoded_width, decoded_height) catch unreachable;
    std.debug.assert(payload.len == decoded_len);
}
...
fn decodedRawOwned(
    ...
    decoded_height: u32,
    payload: []const u8,
) host_state.ApplyError![]u8 {
    assertDecodedPayloadCoherent(format, width, height, decoded_format, decoded_width, decoded_height, payload);
    std.debug.assert(format == 24 or format == 32);
    std.debug.assert(decoded_format == format);
...
fn decodedRgbaOwned(
    ...
    assertDecodedPayloadCoherent(format, width, height, decoded_format, decoded_width, decoded_height, payload);
    if (decoded_format == 32) return try allocator.dupe(u8, payload);
    std.debug.assert(decoded_format == 24);
    return try expandRgbRawOwned(allocator, width, height, payload);
}

fn copyRawOwned(allocator: std.mem.Allocator, format: u16, width: u32, height: u32, payload: []const u8) host_state.ApplyError![]u8 {
    const raw_len = expectedRawPayloadLen(format, width, height) catch unreachable;
    std.debug.assert(payload.len == raw_len);
    return try allocator.dupe(u8, payload);
}

fn framePublicationFormatSupported(format: u16) bool {
...
fn decodedPublicationOwned(
    ...
    if (format == 100) {
        const decoded = decodePngRgbaOwned(allocator, width, height, payload) catch |err| switch (err) {
            error.InvalidPngData => return error.ConsequenceLimit,
            error.InvalidGraphicsData => return error.ConsequenceLimit,
            error.OutOfMemory => return error.OutOfMemory,
            error.ConsequenceLimit => return error.ConsequenceLimit,
    ...
    const decoded = try copyRawOwned(allocator, format, width, height, payload);
    return .{ .format = format, .width = width, .height = height, .payload = decoded };
}

fn expandRgbRawOwned(allocator: std.mem.Allocator, width: u32, height: u32, rgb: []const u8) host_state.ApplyError![]u8 {
    const rgb_len = expectedRawPayloadLen(24, width, height) catch unreachable;
    std.debug.assert(rgb.len == rgb_len);
...
fn decodePngRgbaOwned(allocator: std.mem.Allocator, width: u32, height: u32, payload: []const u8) (PngDecodeError || host_state.ApplyError)![]u8 {
    var image_size = try decodePngSize(payload);
    const ptr = c.stbi_load_from_memory(payload.ptr, @intCast(payload.len), &image_size.width, &image_size.height, &image_size.comp, 4) orelse return error.InvalidPngData;
...
fn intrinsicImageSize(format: u16, width: u32, height: u32, payload: []const u8) (PngDecodeError || host_state.ApplyError)!struct { width: u32, height: u32 } {
    ...
    const image_size = try decodePngSize(payload);
...
fn loadIndirectPayloadNormalized(allocator: std.mem.Allocator, cmd: KittyGraphicsCommand) (MediaLoadError || host_state.ApplyError)![]u8 {
    ...
    if (cmd.compression == 0) {
        ...
        try ensureDecodedPayloadStoreForLen(payload.len);
        return try allocator.dupe(u8, payload);
    }
    return try decompressRawPayloadOwned(allocator, cmd.format, cmd.width, cmd.height, transport);
}
...
fn decodeGraphicsLocator(allocator: std.mem.Allocator, payload: []const u8) (MediaLoadError || host_state.ApplyError)![]u8 {
    ...
    try ensureDecodedPayloadStoreForLen(decoded_len);
...
fn readGraphicsBytes(allocator: std.mem.Allocator, fd: c_int, file_size: u64, cmd: KittyGraphicsCommand) (MediaLoadError || host_state.ApplyError)![]u8 {
    ...
    try ensureDecodedPayloadStoreForLen(read_len);
...
fn decodeKittyDirectChunkOwned(allocator: std.mem.Allocator, payload: []const u8) (error{InvalidGraphicsData} || host_state.ApplyError)![]u8 {
    ...
    try ensureDecodedPayloadStoreForLen(decoded_bound);
...
fn normalizeDirectPayloadOwned(allocator: std.mem.Allocator, compression: u8, format: u16, width: u32, height: u32, payload: []const u8) (DirectPayloadError || host_state.ApplyError)![]u8 {
    const decoded_len = std.base64.standard.Decoder.calcSizeForSlice(payload) catch {
        if (format == 24 or format == 32) return error.InvalidRawGraphicsData;
        return error.InvalidGraphicsData;
    };
    try ensureDecodedPayloadStoreForLen(decoded_len);
    const decoded = try allocator.alloc(u8, decoded_len);
    ...
    return try normalizeDirectTransportOwned(allocator, compression, format, width, height, decoded);
}

fn normalizeDirectTransportOwned(allocator: std.mem.Allocator, compression: u8, format: u16, width: u32, height: u32, transport: []const u8) (DirectPayloadError || host_state.ApplyError)![]u8 {
    if (compression == 0) {
        if (format == 100) try validatePngBytes(transport);
        if (format == 24 or format == 32) return try normalizeRawPayloadOwned(allocator, format, width, height, transport);
        return try allocator.dupe(u8, transport);
    }
    if (compression != 'z') return error.InvalidGraphicsCompression;
    return try decompressRawPayloadOwned(allocator, format, width, height, transport);
}

fn normalizeRawPayloadOwned(
    allocator: std.mem.Allocator,
    format: u16,
    width: u32,
    height: u32,
    ...
    if (transport.len < raw_len) return error.InvalidRawGraphicsData;
    if (transport.len - raw_len > 10) return error.TooMuchData;
    return try allocator.dupe(u8, transport[0..raw_len]);
}
