...
test "screen: placeholder lead cell retains yazi column diacritic beyond basic block" {
    const gpa = std.testing.allocator;
    var s = try Screen.initWithCells(gpa, 2, 4);
    defer s.deinit(gpa);
    ...
    s.apply(SemanticEvent{ .write_codepoint = 0x10EEEE });
    s.apply(SemanticEvent{ .write_codepoint = 0x0305 });
    s.apply(SemanticEvent{ .write_codepoint = 0x0483 });

    const cell = s.cellInfoAt(0, 0);
    try std.testing.expectEqual(@as(u21, 0x10EEEE), cell.codepoint);
    try std.testing.expectEqual(@as(u8, 2), cell.combining_len);
    try std.testing.expectEqualSlices(u32, &.{ 0x0305, 0x0483 }, cell.combining[0..2]);
    try std.testing.expectEqual(@as(u16, 0), s.cursor_row);
    try std.testing.expectEqual(@as(u16, 1), s.cursor_col);
    try std.testing.expectEqual(@as(u21, 0), s.cellAt(0, 1));
}

test "screen: placeholder lead cell retains yazi arabic row diacritic" {
    const gpa = std.testing.allocator;
    var s = try Screen.initWithCells(gpa, 2, 4);
    defer s.deinit(gpa);

    s.apply(SemanticEvent{ .write_codepoint = 0x10EEEE });
    s.apply(SemanticEvent{ .write_codepoint = 0x0657 });
    s.apply(SemanticEvent{ .write_codepoint = 0x0305 });

    const cell = s.cellInfoAt(0, 0);
    try std.testing.expectEqual(@as(u21, 0x10EEEE), cell.codepoint);
    try std.testing.expectEqual(@as(u8, 2), cell.combining_len);
    try std.testing.expectEqualSlices(u32, &.{ 0x0657, 0x0305 }, cell.combining[0..2]);
    try std.testing.expectEqual(@as(u16, 0), s.cursor_row);
    try std.testing.expectEqual(@as(u16, 1), s.cursor_col);
    try std.testing.expectEqual(@as(u21, 0), s.cellAt(0, 1));
}

test "screen: placeholder diacritics attach to preceding placeholder over non-empty cells" {
    const gpa = std.testing.allocator;
    var s = try Screen.initWithCells(gpa, 2, 4);
    defer s.deinit(gpa);
    ...
    s.apply(SemanticEvent{ .write_codepoint = 0x10EEEE });
    s.apply(SemanticEvent{ .write_codepoint = 0x030D });
    s.apply(SemanticEvent{ .write_codepoint = 0x0610 });

    const placeholder = s.cellInfoAt(0, 0);
    const next = s.cellInfoAt(0, 1);
    try std.testing.expectEqual(@as(u21, 0x10EEEE), placeholder.codepoint);
    try std.testing.expectEqual(@as(u8, 2), placeholder.combining_len);
    try std.testing.expectEqualSlices(u32, &.{ 0x030D, 0x0610 }, placeholder.combining[0..2]);
    try std.testing.expectEqual(@as(u21, 'b'), next.codepoint);
    try std.testing.expectEqual(@as(u8, 0), next.combining_len);
    try std.testing.expectEqual(@as(u16, 0), s.cursor_row);
    try std.testing.expectEqual(@as(u16, 1), s.cursor_col);
}
...
test "screen: DECST8C restores default tab stops" {
    const gpa = std.testing.allocator;
    var s = try Grid.initWithCells(gpa, 1, 20);
    defer s.deinit(gpa);
    s.cursor_col = 8;
