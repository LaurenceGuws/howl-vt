...
pub fn terminalQueryVisibleInfo(vt_handle: handle.VtHandle) callconv(.c) FfiVisibleInfoResult {
    const owned = handle.vtFromHandle(vt_handle) orelse return .{ .status = @intFromEnum(status.HowlVtCallStatus.missing_handle) };
    const meta = owned.visibleMeta();
    return .{
        ...
        .info = .{
            ...
            .scrollback_offset = owned.scrollback_offset,
