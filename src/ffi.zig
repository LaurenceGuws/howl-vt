...
fn applySelectionRangesToSurfaceCells(view: screen_set.View, selected: ?selection.TerminalSelection, cells: []FfiSurfaceCell) void {
...
pub fn terminalCopySelection(handle: VtHandle, ptr: ?[*]u8, cap: usize) callconv(.c) FfiBytesResult {
    const owned = vtFromHandle(handle) orelse return .{ .status = @intFromEnum(HowlVtCallStatus.missing_handle) };
    const out = bytesOut(ptr, cap) orelse return .{ .status = @intFromEnum(HowlVtCallStatus.invalid_argument) };
    const text = selection_projection.copyText(owned.allocator, &owned.screen_state, owned.selectionState()) catch return .{ .status = @intFromEnum(HowlVtCallStatus.failed) };
...
test "vt ffi alternate selection does not read primary history" {
    ...
    const primary_feed = terminalFeed(handle, "aa\r\nbb\r\ncc".ptr, 10);
    try std.testing.expectEqual(@as(i32, @intFromEnum(HowlVtCallStatus.ok)), primary_feed.status);

    const enter_alt = terminalFeed(handle, "\x1b[?1049hzz".ptr, 10);
    try std.testing.expectEqual(@as(i32, @intFromEnum(HowlVtCallStatus.ok)), enter_alt.status);
