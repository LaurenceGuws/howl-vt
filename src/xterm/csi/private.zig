...
pub fn process(final: u8, params: []const i32, leader: u8, intermediates: []const u8) ?SemanticEvent {
    ...
    if (leader == '?' and final == 'g') return SemanticEvent{ .key_format_query = @intCast(@min(params_mod.paramAtOrDefault0(params, 0), std.math.maxInt(u8))) };
    if (leader == '?' and final == 'J') return SemanticEvent{ .selective_erase_display = params_mod.eraseMode(params_mod.paramAtOrDefault0(params, 0)) };
    if (leader == '?' and final == 'K') return SemanticEvent{ .selective_erase_line = params_mod.eraseMode(params_mod.paramAtOrDefault0(params, 0)) };
    if (leader == '?' and final == 'W' and params_mod.paramAtOrDefault0(params, 0) == 5) return SemanticEvent.reset_default_tab_stops;
    if (leader == '?' and params.len >= 1) {
        ...
        if (final == 's' and intermediates.len == 0) return SemanticEvent{ .dec_mode_save = params_mod.collectParams(params) };
        if (final == 'r' and intermediates.len == 0) return SemanticEvent{ .dec_mode_restore = params_mod.collectParams(params) };
        if (final == 'i') return SemanticEvent{ .media_copy_request = params_mod.paramAtOrDefault0(params, 0) };
        if (final == 'n') return switch (params_mod.paramAtOrDefault0(params, 0)) {
