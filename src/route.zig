...
fn applySemantic(vt: anytype, event: SemanticEvent) host_state.ApplyError!bool {
    ...
    switch (event) {
        ...
        .displayed_extent_report => try report_apply.apply(vt, .displayed_extent_report),
    ...
    switch (event) {
        ...
        .erase_display => |v| vt.screen_state.active().applyScreen(.{ .erase_display = v }),
        .erase_line => |v| vt.screen_state.active().applyScreen(.{ .erase_line = v }),
        .selective_erase_display => |v| vt.screen_state.active().applyScreen(.{ .selective_erase_display = v }),
